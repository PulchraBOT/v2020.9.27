...
class MusicPlayer(commands.Cog, name='Müzik'):
    ...
    async def start_song(self, msg, song, spotify=False, member=None, sayı=None, image=None, url=None):
        async with msg.channel.typing():
            try:
                ...
                if spotify == True:
                    mesaj = f"**Spotify Linki:** [SpotifyURL]({url})\n"
                    ...
                    emb = discord.Embed(colour=msg.guild.me.color,description=mesaj, title=song)
    ...
    @command(aliases=["sp"])
    async def spotify(self, msg, member: discord.Member=None):
        await Logger.guildLogger(self, msg)
        if member is None:
            member = msg.author
        index = 0
                ...
                while True:
                    ...
                    try:
                        if member.activities[index].title != title:
                            ...
                            if msg.guild.id in self.player:
                                if msg.voice_client.is_playing() is False and not self.player[msg.guild.id]['queue']:
                                    await self.start_song(msg, song, True, member, count, image, url)
                                    count += 1
                                    continue
        ...
        async with msg.channel.typing():
            ...
            for i in data['entries']:
                ...
                if str(reaction.emoji) == '\u2705':  # check
                    ...
                    return await self.çal(msg=msg, song_url=i['webpage_url'])
